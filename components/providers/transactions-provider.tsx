...
export function TransactionsProvider({ children }: { children: React.ReactNode }) {
            ...
            try {
                ...
                if (isOnline && token) {
                    try {
                        ...
                        console.error("Failed to fetch sales from API, falling back to local", e);
                    }
                }
                ...
                if (isOnline && token) {
                    try {
                        const apiExpenses = await api.get("/expenses");
    ...
    const addExpense = async (newExpense: Omit<Expense, "id" | "date">) => {
        ...
        try {
            if (isOnline) {
                const savedExpense = await api.post("/expenses", expense);
            ...
            logEvent("create", "expense", `Registró un gasto de $${expense.amount.toLocaleString()}: ${expense.description}`, tempId)
        } catch (error) {
    ...
    const addSale = async (newSale: Omit<Sale, "id" | "date">) => {
        ...
        try {
            ...
            if (isOnline) {
                try {
                    await api.post("/sales", { ...sale, id: saleId });
                    ...
                    setSales(prev => prev.map(s => s.id === saleId ? { ...s, status: 'PENDING' } : s));
                }
            } else {
                await db.pendingActions.add({
                    id: crypto.randomUUID(),
